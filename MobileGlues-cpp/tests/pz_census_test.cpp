...
int main() {
    ...
    setenv("MOBILEGLUES_PZ_GPU_BUFFER_POOL", "0", 1);
    ...
    expect(!mg_pz_gpu_buffer_pool_active, "0 must disable the GPU buffer pool");
    ...
    setenv("MOBILEGLUES_PZ_GPU_BUFFER_POOL", "true", 1);
    ...
    expect(!mg_pz_gpu_buffer_pool_active, "only the exact value 1 may enable the GPU buffer pool");
    ...
    setenv("MOBILEGLUES_PZ_GPU_BUFFER_POOL", "1", 1);
    ...
    expect(mg_pz_gpu_buffer_pool_active, "1 must enable the GPU buffer pool");
    ...
    expect(!mg_pz_census_active && !mg_pz_vao_fastpath_active && !mg_pz_attrib_fastpath_active &&
               ...
               !mg_pz_buffer_discard_coalesce_active && !mg_pz_gpu_buffer_pool_active &&
               !mg_pz_state_shadow_active && !mg_pz_runtime_mipmap_skip_active,
