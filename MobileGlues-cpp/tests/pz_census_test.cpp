...
int main() {
    ...
    unsetenv("MOBILEGLUES_PZ_UNIFORM_LOCATION_CACHE");
    unsetenv("MOBILEGLUES_PZ_ASYNC_ERROR_DRAIN");
    ...
    expect(!mg_pz_uniform_location_cache_active, "uniform-location cache must remain experimental and opt-in");
    expect(!mg_pz_async_error_drain_active, "async error drain must remain experimental and opt-in");
    ...
    setenv("MOBILEGLUES_PZ_UNIFORM_LOCATION_CACHE", "0", 1);
    setenv("MOBILEGLUES_PZ_ASYNC_ERROR_DRAIN", "0", 1);
    ...
    expect(!mg_pz_uniform_location_cache_active, "0 must disable the uniform-location cache");
    expect(!mg_pz_async_error_drain_active, "0 must disable async error drain");
    ...
    setenv("MOBILEGLUES_PZ_UNIFORM_LOCATION_CACHE", "true", 1);
    setenv("MOBILEGLUES_PZ_ASYNC_ERROR_DRAIN", "true", 1);
    ...
    expect(!mg_pz_uniform_location_cache_active,
           "only the exact value 1 may enable the uniform-location cache");
    expect(!mg_pz_async_error_drain_active, "only the exact value 1 may enable async error drain");
    ...
    setenv("MOBILEGLUES_PZ_UNIFORM_LOCATION_CACHE", "1", 1);
    setenv("MOBILEGLUES_PZ_ASYNC_ERROR_DRAIN", "1", 1);
    ...
    expect(mg_pz_uniform_location_cache_active, "1 must enable the uniform-location cache");
    expect(mg_pz_async_error_drain_active, "1 must enable async error drain with threaded submission");
    ...
    mg_pz_census_uniform_location(false, 1, true);
    mg_pz_census_uniform_location(true, 0, false);
    mg_pz_census_async_error_drain(true);
    mg_pz_census_async_error_drain(false);
    ...
    expect(last_file_log.find("schema=8") != std::string::npos, "schema 8 must be reported");
    ...
    expect(last_file_log.find("uniform_loc=2/1/1/1") != std::string::npos,
           "uniform-location requests, cache hits, driver queries and stores must be reported");
    expect(last_file_log.find("error_drain=2/1/1") != std::string::npos,
           "async error drain requests, submissions and fallbacks must be reported");
    ...
    expect(!mg_pz_census_active && !mg_pz_vao_fastpath_active && !mg_pz_attrib_fastpath_active &&
               ...
               !mg_pz_large_uniform_async_active && !mg_pz_uniform_location_cache_active &&
