...
bool mg_pz_chunk_early_discard_active = false;
...
void mg_pz_census_init(void) {
    ...
    const char* chunk_early_discard_value = std::getenv("MOBILEGLUES_PZ_CHUNK_EARLY_DISCARD");
    mg_pz_chunk_early_discard_active =
        chunk_early_discard_value != nullptr && std::strcmp(chunk_early_discard_value, "1") == 0;
    g_census = {};
    ...
    if (mg_pz_census_active) {
        ...
        if (mg_pz_chunk_early_discard_active)
            LOG_I("ZOMDROID_PZ_CHUNK_EARLY_DISCARD enabled=1 mode=alpha_before_depth_fetch")
    }
#endif
}
