...
void mg_pz_census_init(void) {
    ...
    const char* gpu_ring_value = std::getenv("MOBILEGLUES_PZ_GPU_BUFFER_POOL");
    mg_pz_gpu_buffer_pool_active = gpu_ring_value != nullptr && std::strcmp(gpu_ring_value, "1") == 0;
    ...
    if (mg_pz_gpu_buffer_pool_active) LOG_I("ZOMDROID_PZ_GPU_BUFFER_POOL enabled=1 depth=3")
