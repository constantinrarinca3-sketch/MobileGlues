...
constexpr int k_pz_tile_batch_max_runs = 8;
...
inline pz_tile_batch_rewrite_result rewrite_pz_default_tile_batch(std::string& glsl) {
    ...
    glsl.insert(z_declaration + 1,
                ...
                "\nuniform int zomdroidBatchRunStart[8];"
                "\nuniform vec2 zomdroidBatchDepth[8];");
    ...
    const std::string replacement =
        ...
        "        for (int zomdroidRun = 1; zomdroidRun < 8; ++zomdroidRun) {\n"
